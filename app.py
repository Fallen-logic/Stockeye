...
# Finnhub symbol map (BSE/NSE symbols in Finnhub format)
FINNHUB_MAP = {
    "TCS":       "BSE:TCS",
    "SBI":       "BSE:SBIN",
    "HDFC":      "BSE:HDFCBANK",
...
FINNHUB_KEY = "d6hj219r01qr5k4c5mggd6hj219r01qr5k4c5mh0"
...
def get_stats(ticker):
    ...
    try:
        fh_sym = FINNHUB_MAP.get(ticker.upper())

        def fmt_cr(val):
            try:
                if val and val > 0:
                    # Finnhub returns market cap in millions USD, convert to INR Cr approx
                    inr = val * 1e6 * 84  # 1 USD ~ 84 INR
                    return f"₹{round(inr/1e7, 0):,.0f} Cr"
            except: pass
            return "—"
        ...
        if fh_sym:
            try:
                r1 = req.get(f"{base}/stock/metric?symbol={fh_sym}&metric=all", headers=headers, timeout=10)
                metric = r1.json().get("metric", {})
            except: pass
            try:
                r2 = req.get(f"{base}/stock/profile2?symbol={fh_sym}", headers=headers, timeout=10)
                profile = r2.json()
            except: pass

        # Also get volume from yfinance fast_info as fallback
        vol = "—"
        try:
            fi = yf.Ticker(yf_sym).fast_info
            vol = fmt_vol(getattr(fi, "three_month_average_volume", None))
        except: pass

        result = {
            "ticker":      ticker,
            "market_cap":  fmt_cr(profile.get("marketCapitalization")),
            "pe_ratio":    fmt_f(metric.get("peBasicExclExtraTTM")),
            "eps":         fmt_f(metric.get("epsBasicExclExtraAnnual")),
            "week52_high": f"₹{fmt_f(metric.get('52WeekHigh'))}" if metric.get("52WeekHigh") else "—",
            "week52_low":  f"₹{fmt_f(metric.get('52WeekLow'))}"  if metric.get("52WeekLow")  else "—",
            "beta":        fmt_f(metric.get("beta")),
            "div_yield":   fmt_f(metric.get("dividendYieldIndicatedAnnual")) + "%" if metric.get("dividendYieldIndicatedAnnual") else "—",
            "volume":      vol,
            "avg_volume":  vol,
